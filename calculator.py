...
class Calculator(object):
    ...
    def loading(self, temp, chempot, mask=None, MBWR=False):
        ...
        """
        fn = '%s/rho_%7.5fkJmol_%7.5fK.npy' %(self.workdir, chempot/kjmol, temp/kelvin)
    ...
    def get_chemical_potential(self, temps):
        ...
        for T in temps:
            chempots = []
            with open(os.path.join(self.workdir + "/name_file_%3.0fK.txt"%(T))) as n:
                for x in n:
                    l = x.split(",")
    ...
    def save_loadings(self, Temps=None, chempot_dict=None, pressure=False, eos=None, AIF=False):
        """
        Saves the loadings of all the calculated densities at the specified temperatures in a csv file vs the chemical potential or pressure

        Parameters
        ----------
        Temps: list of temperatures
        chempot_dict: a dictionary containing the lsit of temepratures and the corresponding chemical potentials, for which the loading is wanted

        """

        if Temps is None:
            namefiles = [fn for fn in os.listdir(self.workdir) if fn.startswith('name_file')]
    
            Temps = []
            for file_name in namefiles:
                numeric_const_pattern = '[-+]? (?: (?: \d* \. \d+ ) | (?: \d+ \.? ) )(?: [Ee] [+-]? \d+ ) ?'
                rx = re.compile(numeric_const_pattern, re.VERBOSE)
                Temps.append(float(rx.findall(file_name)[0]))
        ...
        if AIF:
            ...
            for T in Temps:
                ...
                if pressure:
                    np.savetxt(self.workdir + '/loads_%3.0fK_vs_P.csv'%T, load_chem, delimiter=',', header='pressures, loadings', comments='')
                else:    
                    np.savetxt(self.workdir + '/loads_%3.0fK.csv'%T, load_chem, delimiter=',', header='chempot, loadings', comments='')
    ...
    def free_energy_contrib(self, temp, chempot, partname, over_loading=False, local=False, fn=None):
        ...
        if fn is None:
            fn = '%s/rho_%7.5fkJmol_%7.5fK.npy' %(self.workdir, chempot/kjmol, temp/kelvin)
            assert os.path.isfile(fn), 'No density found for %7.5f K and %7.5f kJ/mol' %(temp,chempot/kjmol)
    ...
    def grand_potential(self, temp, chempot):
        ...
        ----------
        temp
            temperature at which the grand potential is being calculated
        chempot
            The chemical of the system
        
        Returns
        -------
            The function `grand_potential` is returning the grand potential of the system at a given
        temperature and chemical potential. The grand potential is calculated by subtracting the product of
        the chemical potential and the loading (the amount of particles in the system) from the free energy
        of the system.
    ...
    def free_energy_path(self, temp, chempot, chempots=None, diffusion_path=None, ring_indices=None, dist_from_axis=None, supercell=False, nbins=100):
        ...
        if chempots is None:           #if no list of chemical potentials is provided it is found through the name_file
            nf_fn = self.workdir + '/name_file_%3.0fK.txt'%(temp)
            list_chems = []
            with open(nf_fn) as f:
                lines = f.readlines()
                for line in lines:
                    list_chems.append(float(line.translate({ord('\n'): None}).split(',')[-1]))
            list_chems = list(selection_sort(np.array(list_chems)))
        else:
            list_chems = list(selection_sort(np.array(list_chems)))
        ...
        omega_list =  np.empty(cvs.shape, dtype=np.float128)
        ...
        for e in range(nbins-1):  # now calculating n and p for the different input collective variables
            ...
            def calc_n(chempotential):
                fn = '%s/rho_%4.5fkJmol_%3.0fK.npy' %(self.workdir, chempotential/kjmol, temp/kelvin)
                assert os.path.isfile(fn), 'No density found for %3.0f K and %4.5f kJ/mol' %(temp,chempotential/kjmol)
                rho = np.load(fn).real
                if supercell:
        ...
        data = np.empty((4,len(cvs)))
        data[:] = cvs, n_list, p_list, omega_list
        np.savetxt(self.workdir + f'/free_energy_profile_{chempot/kjmol:#0.8f}kjmol_{temp:#0.3f}K.csv', data.T, delimiter=',', header = 'cv, density, density probability, grand canonical potential')        
        # return cvs_mat
        return cvs, n_list, p_list, omega_list
    ...
    def diffusion_constant(self, chempot, temperature, dT=0.001*kelvin, alpha=0.788, weighted_density=False):
        ...
        with log.section('PROGRAM', 2, timer='Diffusion constant'):
            ...
            assert os.path.isfile(fn), 'No density found for %3.0f K and %4.5f kJ/mol' %(T2,chempot/kjmol)  
            rho = np.load(fn)           
